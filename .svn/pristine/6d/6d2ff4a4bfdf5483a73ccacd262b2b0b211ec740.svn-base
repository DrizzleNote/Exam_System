                            ...
                            <div class="col-sm-12">
                                <table id="data_table" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="data_table_info">
                                    ...
                                    foreach ($info as $model) {
                                        echo '<tr id="rowid_' . $model->ExamConfigRecordID . '">';
                                        echo '  <td><label><input type="checkbox" value="' . $model->ExamConfigRecordID . '"></label></td>';
                                        echo '  <td>' . $model->ExamPaperName. '</td>';
                                        echo '  <td>' . $model->ConfigTeacherName . '</td>';
                                        echo '  <td>' . $model->ConfigTime . '</td>';
                                        echo '  <td class="center">';
                                        echo '      <a id="view_btn" onclick="viewAction(' . "'$model->ExamConfigRecordID'". ')" class="btn btn-primary btn-sm" href="#"> <i class="glyphicon glyphicon-zoom-in icon-white"></i>查看</a>';
                                        echo '      <a id="edit_btn" onclick="editNameAction(' . "'$model->ExamConfigRecordID'"  . ')" class="btn btn-primary btn-sm" href="#"> <i class="glyphicon glyphicon-edit icon-white"></i>修改模块名称</a>';
                                        echo '      <a id="delete_btn" onclick="deleteAction(' . "'$model->ExamConfigRecordID'"  . ')" class="btn btn-danger btn-sm" href="#"> <i class="glyphicon glyphicon-trash icon-white"></i>删除</a>';
                                        echo '  </td>';
                                        echo '<tr/>';
                                    }
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <td>备注:</td>
                                <td colspan="3" id="view-remark"></td>
                            </tr>
                            <tr id="view-detail">
                                <th>题型</th>
                                <th>题目数量</th>
    ...
    function initModel(id, type, fun){
        $.ajax({
            ...
            success: function(data){
                console.log(data)
                $('#view-name').text(data['tmp']['ExamPaperName']);
                $('#view-teacher').text(data['tmp']['ConfigTeacherName']);
                $('#view-totalScore').text(data['totalScore']);
                $('#view-item').text(data['tmp']['CourseID']);
                $('#view-time').text(data['tmp']['ConfigTime']);
                ...
                for(var tmp in data['data']){
                    $('#view-detail').after('<tr name="class"><td>'+ data['data'][tmp]['QuestionType'] +'</td>' +
                        ...
                        '<td>'+ data['data'][tmp]['EveryQuestionSocre'] +'</td>' +
                        '<td>'+ data['data'][tmp]['difficulty'] +'</td>' +
                        '<td>'+ data['data'][tmp]['stage'] +'</td></tr>');
                }
