...
class LogController extends  BaseController{
    ...
    public function actionSearchLog()
    {
        ...
        if(\Yii::$app->request->isPost)
        {
            ...
            if(isset($post['key']))
            {
                $info = AdminLog::find()->select("id,route,user_agent,admin_id,ip,user,created_at")->where("route like :key or ip like :key or user like :key",[':key'=>'%'.$post['key'].'%'])->orderBy("created_at DESC")->offset($pagesize*(int)$post['PageNum'])
                ->limit($pagesize)->asArray()->all();
