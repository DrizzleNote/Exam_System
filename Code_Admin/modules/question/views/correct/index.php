    ...
    var ue = UE.getEditor('correct_Description');
      ...
      function getKnowledge(stageId)
        ...
        $('#stage-choice').change(function () {
            var Tmp = $(this).val();
            window.location.href = '<?=Url::toRoute('correct/index')?>'+'&stage='+Tmp;
        })
            ...
            function(res){
                ...
                for(var key in res)
                {
                    ...
                    for(var key1 in res[key].Answer.key)
                    {
                        ...
                        html += '<div class="col-sm-6">'+
                                '<input type="text" class="form-control"  value="'+res[key].Answer.key[key1].Answer+'" name="old['+res[key].ErrorCount+']['+key1+']" placeholder="必填"/>'+
                                '</div>';
                    }
                    html += '</th>'+
